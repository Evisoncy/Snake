...
class aplicacion_i:
    ...
    def on_loop(self):
        ...
        for i in range(0,self.computadora_i.longitud):
            if self.juego.isCollision(self.manzana.x,self.manzana.y,self.computadora_i.x[i], self.computadora_i.y[i],44):
                self.manzana.x = randint(2,9) * 44
                self.manzana.y = randint(2,9) * 44
    ...
    def on_execute(self):
        ...
        while( self._running ):
            ...
            if (keys[K_KP_3]):
                self.computadora_i.moveRight()
                
            if (keys[K_KP_1]):
                self.computadora_i.moveLeft()
                
            if (keys[K_KP_5]):
                self.computadora_i.moveUp()
                
            if (keys[K_KP_2]):
                self.computadora_i.moveDown()
